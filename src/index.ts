...
function* script(r: SberRequest) {
  ...
  load(url).then(temp => { if (temp.waterMax === undefined) { state.loaded = true; upload(url, state); } else { if(state.date === temp.date) {state.waterCount += temp.waterCount; state.waterMax += temp.waterMax;} state.date = temp.date; state.loaded = true; }});
  ...
  while (true) {
    if(r.type === 'SERVER_ACTION')
    {
      ...
      let splitted = r.msg.toLowerCase().split(" ");
      let numbered = 0;
